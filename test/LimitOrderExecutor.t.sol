...
contract LimitOrderExecutorTest is DSTest {
    ...
    //
    address TAXED_TOKEN_2 = 0xd99793A840cB0606456916d1CF5eA199ED93Bf97;
    //6% tax CHAOS token 27
    ...
    function setUp() public {
        ...
        limitOrderRouter = ILimitOrderRouter(limitOrderExecutor.LIMIT_ORDER_ROUTER());
    ...
    function testExecuteTokenToWethOrderBatch() public {
        ...
        for (uint256 i = 0; i < tokenToWethOrderBatch.length; ++i) {
            LimitOrderBook.LimitOrder memory order0 = orderBook.getLimitOrderById(tokenToWethOrderBatch[i]);
        ...
        }
        {
            ...
            for (uint256 i = 0; i < tokenToWethOrderBatch.length; ++i) {
                ...
                assertGe(gasCompensationAfter - gasCompensationBefore, cumulativeGasCompensation);

                LimitOrderBook.OrderType orderType =
                    orderBook.addressToOrderIds(address(this), tokenToWethOrderBatch[i]);
    ...
    function testFailExecuteTokenToWethOrderBatch_DuplicateOrdersInExecution() public {
        ...
        for (uint256 i = 0; i < tokenToWethOrderBatch.length; ++i) {
            LimitOrderBook.LimitOrder memory order = orderBook.getLimitOrderById(tokenToWethOrderBatch[i]);
    ...
    function testExecuteWethToTokenSingle() public {
        ...
        cheatCodes.prank(tx.origin);
        limitOrderExecutor.checkIn();
        (bool depositSuccess,) = address(WETH).call{value: 500000000000 ether}(abi.encodeWithSignature("deposit()"));
        ...
        for (uint256 i = 0; i < orderBatch.length; ++i) {
            LimitOrderBook.LimitOrder memory order0 = orderBook.getLimitOrderById(orderBatch[i]);
        ...
        // check that the orders have been fufilled and removed
        for (uint256 i = 0; i < orderBatch.length; ++i) {
            LimitOrderBook.OrderType orderType = orderBook.addressToOrderIds(address(this), orderId);
            assert(orderType == LimitOrderBook.OrderType.FilledLimitOrder);
        }
    }
    ...
    function testExecuteTokenToWethSingle(uint112 amountIn) public {
        ...
        if (run) {
            ...
            (LimitOrderSwapRouter.SpotReserve memory spRes,) =
                limitOrderExecutor.calculateV3SpotPrice(DAI, WETH, 500, 0x1F98431c8aD98523631AE4a59f267346ea31F984);
            ...
            uint112 amountOutMin =
                uint112(ConveyorMath.mul64U(_95_PERCENT, ConveyorMath.mul128U(spRes.spotPrice, amountIn)));
            ...
            //Should be fee
            uint256 balanceAfterMin = ConveyorMath.mul64U(ConveyorMath.div64x64(uint128(1), fee), amountOutMin);

            {
                ...
                for (uint256 i = 0; i < orderBatch.length; ++i) {
                    ...
                    ///@notice Ensure the upper and lower execution thresholds were paid to the beacon.
                    assertEq(gasCompensationAfter - gasCompensationBefore, cumulativeGasCompensation);
    ...
    function testExecuteWethToTokenOrderBatch() public {
        ...
        (bool depositSuccess,) = address(WETH).call{value: 500000000 ether}(abi.encodeWithSignature("deposit()"));
    ...
    function testExecuteTokenToTokenSingle(uint80 amountIn) public {
        ...
        if (run) {
            ...
            ///@notice Get the spot price of DAI/WETH
            (LimitOrderSwapRouter.SpotReserve memory spRes,) =
                limitOrderExecutor.calculateV3SpotPrice(DAI, UNI, 500, 0x1F98431c8aD98523631AE4a59f267346ea31F984);
    ...
    function testExecuteTokenToTokenBatch() public {
        ...
        for (uint256 i = 0; i < tokenToTokenOrderBatch.length; ++i) {
            LimitOrderBook.LimitOrder memory order0 = orderBook.getLimitOrderById(tokenToTokenOrderBatch[i]);
            cumulativeGasCompensation += order0.executionCredit;
            assert(order0.orderId != bytes32(0));
        }
        {
            ...
            for (uint256 i = 0; i < tokenToTokenOrderBatch.length; ++i) {
                ...
                LimitOrderBook.OrderType orderType =
                    orderBook.addressToOrderIds(address(this), tokenToTokenOrderBatch[i]);
                assert(orderType == LimitOrderBook.OrderType.FilledLimitOrder);
            }
        }
    }
    ...
    function testExecuteWethToTaxedTokenSingle(uint112 amountIn) public {
        ...
        if (run) {
            ...
            address(WETH).call{value: amountIn}(abi.encodeWithSignature("deposit()"));

            IERC20(WETH).approve(address(limitOrderExecutor), MAX_UINT);

            ///@notice Get the spot price of DAI/WETH
            (LimitOrderSwapRouter.SpotReserve memory spRes,) =
                limitOrderExecutor.calculateV2SpotPrice(WETH, TAXED_TOKEN, _uniV2FactoryAddress);
...

            {
                ...
                LimitOrderBook.LimitOrder memory order =
                    newMockOrder(WETH, TAXED_TOKEN, 1, false, true, 0, amountOutMin, amountIn, 3000, 0, 0, MAX_U32);
    ...
    function testExecuteWethToTaxedTokenBatch() public {
        ...
        for (uint256 i = 0; i < wethToTaxedOrderBatch.length; ++i) {
            LimitOrderBook.LimitOrder memory order0 = orderBook.getLimitOrderById(wethToTaxedOrderBatch[i]);
            cumulativeGasCompensation += order0.executionCredit;
            assert(order0.orderId != bytes32(0));
        }
        {
            ...
            for (uint256 i = 0; i < wethToTaxedOrderBatch.length; ++i) {
                ...
                LimitOrderBook.OrderType orderType =
                    orderBook.addressToOrderIds(address(this), wethToTaxedOrderBatch[i]);
                assert(orderType == LimitOrderBook.OrderType.FilledLimitOrder);
            }
        }
    }
    ...
    function testExecuteTaxedTokenToTokenBatch() public {
        ...
        }
        {
            ...
            for (uint256 i = 0; i < orderBatch.length; ++i) {
                ...
                LimitOrderBook.OrderType orderType = orderBook.addressToOrderIds(address(this), orderBatch[i]);
                assert(orderType == LimitOrderBook.OrderType.FilledLimitOrder);
            }
        }
    }
    ...
    function testFailExecuteTokenToTokenBatch_InvalidNonEOAStoplossExecution() public {
        ...
        bytes32[] memory tokenToTokenOrderBatch = placeNewMockTokenToTokenStoplossBatch();

        //check that the orders have been placed
        for (uint256 i = 0; i < tokenToTokenOrderBatch.length; ++i) {
            LimitOrderBook.LimitOrder memory order = orderBook.getLimitOrderById(tokenToTokenOrderBatch[i]);
    ...
    function testFailExecuteTokenToTokenBatch_DuplicateOrdersInExecution() public {
        ...
        bytes32[] memory tokenToTokenOrderBatch = placeNewMockTokenToTokenBatchDuplicateOrderIds();
    ...
    //Test fail case in execution with duplicate orderIds passed
    function testFailExecuteTokenToWethOrderBatch_InvalidNonEOAStoplossExecution() public {
        cheatCodes.prank(tx.origin);
        limitOrderExecutor.checkIn();
        cheatCodes.deal(address(swapHelper), MAX_UINT);

        IERC20(DAI).approve(address(limitOrderExecutor), MAX_UINT);
        bytes32[] memory tokenToWethOrderBatch = placeNewMockTokenToWethBatchStoploss();
    ...
    function placeMockOrder(LimitOrderBook.LimitOrder memory order) internal returns (bytes32 orderId) {
        //create a new array of orders
        LimitOrderBook.LimitOrder[] memory orderGroup = new LimitOrderBook.LimitOrder[](1);
        ...
        //place order
        bytes32[] memory orderIds = orderBook.placeLimitOrder{value: type(uint64).max}(orderGroup);
        ...
        returns (bytes32[] memory orderIds)
    {
        ...
        //place order
        orderIds = orderBook.placeLimitOrder{value: type(uint32).max * orderGroup.length}(orderGroup);
    }

    function placeNewMockTokenToWethBatch() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
        ...
        LimitOrderBook.LimitOrder[] memory orderBatch = new LimitOrderBook.LimitOrder[](4);
        orderBatch[0] = order1;
        orderBatch[1] = order2;
        orderBatch[2] = order3;
        orderBatch[3] = order4;
    ...
    function placeNewMockTokenToWethBatchDuplicateOrderIds() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function placeNewMockTokenToWethTaxedBatch() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, TAXED_TOKEN);
    ...
    function placeNewMockTokenToWethBatch_InvalidBatchOrdering() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
        ...
        LimitOrderBook.LimitOrder[] memory orderBatch = new LimitOrderBook.LimitOrder[](3);
        orderBatch[0] = order1;
        orderBatch[1] = order2;
        orderBatch[2] = order3;
    ...
    function newMockTokenToWethBatch_InvalidBatchOrdering() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function placeNewMockTokenToWethBatch_IncongruentTokenIn() internal returns (bytes32[] memory) {
    ...
    function placeNewMockTokenToWethBatch_IncongruentStoploss() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, UNI);
        swapHelper.swapEthForTokenWithUniV2(1000 ether, USDC);
    ...
    function placeNewMockTokenToWethBatchStoploss() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, UNI);
        swapHelper.swapEthForTokenWithUniV2(1000 ether, USDC);
    ...
    function newMockTokenToWethBatch_IncongruentTokenIn() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, UNI);
        swapHelper.swapEthForTokenWithUniV2(1000 ether, USDC);
    ...
    function placeNewMockTokenToWethBatch_IncongruentTaxedTokenInBatch() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function placeNewMockTokenToWethBatch_IncongruentTokenOut() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function newMockTokenToWethBatch_IncongruentTokenOut() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function newMockTokenToWethBatch_IncongruentFeeIn() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
        ...
        LimitOrderBook.LimitOrder[] memory orderBatch = new LimitOrderBook.LimitOrder[](2);
        orderBatch[0] = order1;
        orderBatch[1] = order2;
    ...
    function newMockTokenToWethBatch_IncongruentFeeOut() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function placeNewMockTokenToWethBatch_IncongruentBuySellStatus() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function newMockTokenToWethBatch_IncongruentBuySellStatus() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
        LimitOrderBook.LimitOrder memory order1 = newMockOrder(
            DAI,
            WETH,
    ...
    function placeNewMockWethToTokenBatch() internal returns (bytes32[] memory) {
        LimitOrderBook.LimitOrder memory order1 = newMockOrder(
            WETH,
            DAI,
            1,
        ...
        LimitOrderBook.LimitOrder[] memory orderBatch = new LimitOrderBook.LimitOrder[](6);
        orderBatch[0] = order1;
        orderBatch[1] = order2;
        orderBatch[2] = order3;
        orderBatch[3] = order4;
    ...
    function placeNewMockWethToTaxedBatch() internal returns (bytes32[] memory) {
        LimitOrderBook.LimitOrder memory order = newMockOrder(
            WETH,
            TAXED_TOKEN,
            1,
    ...
    function placeNewMockTokenToTokenBatch() internal returns (bytes32[] memory) {
    ...
    function placeNewMockTokenToTokenStoplossBatch() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(10000 ether, USDC);
    ...
    function placeNewMockTokenToTokenBatchDuplicateOrderIds() internal returns (bytes32[] memory) {
        swapHelper.swapEthForTokenWithUniV2(10000 ether, USDC);
    ...
    function placeNewMockTaxedToTokenBatch() internal returns (bytes32[] memory) {
        LimitOrderBook.LimitOrder memory order = newMockOrder(
            TAXED_TOKEN,
            DAI,
            1,
    ...
    function placeNewMockTaxedToTaxedTokenBatch() internal returns (bytes32[] memory) {
        LimitOrderBook.LimitOrder memory order = newMockOrder(
            TAXED_TOKEN,
            TAXED_TOKEN_1,
            1,
        ...
        LimitOrderBook.LimitOrder[] memory orderBatch = new LimitOrderBook.LimitOrder[](1);
        orderBatch[0] = order;

        return placeMultipleMockOrder(orderBatch);
    }

    function newMockTokenToTokenBatch() internal returns (LimitOrderBook.LimitOrder[] memory) {
        swapHelper.swapEthForTokenWithUniV2(1000 ether, DAI);
    ...
    function newOrder(address tokenIn, address tokenOut, uint128 price, uint128 quantity, uint128 amountOutMin)
        internal
        view
        returns (LimitOrderBook.LimitOrder memory order)
    {
        //Initialize mock order
        order = LimitOrderBook.LimitOrder({
            stoploss: false,
...
contract LimitOrderExecutorWrapper is ConveyorExecutor {
    ...
    ) ConveyorExecutor(_weth, _usdc, _limitOrderQuoter, _dexFactories, _isUniV2, _minExecutionCredit) {}
...
contract LimitOrderRouterWrapper is LimitOrderRouter {
    constructor(address _weth, address _usdc, address _limitOrderExecutor, uint256 _minExecutionCredits)
        LimitOrderRouter(_weth, _usdc, _limitOrderExecutor, _minExecutionCredits)
    {}
