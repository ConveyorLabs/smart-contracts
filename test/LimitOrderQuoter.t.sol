...
contract LimitOrderQuoterTest is DSTest {
    ...
    function testFindBestTokenToTokenExecutionPrice() public {
        ...
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory tokenToTokenExecutionPrice = LimitOrderSwapRouter
        ...
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory tokenToTokenExecutionPrice1 = LimitOrderSwapRouter
            .TokenToTokenExecutionPrice({
            aToWethReserve0: 8014835235973799779324680,
            aToWethReserve1: 4595913824638810919416,
            wethToBReserve0: 1414776373420924126438282,
        ...
        uint256 bestPriceIndexBuy = limitOrderQuoter.findBestTokenToTokenExecutionPrice(executionPrices, true);
        uint256 bestPriceIndexSell = limitOrderQuoter.findBestTokenToTokenExecutionPrice(executionPrices, false);
    ...
    function testSimulateAToBPriceChangeV2ReserveOutputs(uint112 _amountIn) public {
        ...
        if (!underflow) {
            if (_amountIn != 0) {
                (, uint128 reserveA, uint128 reserveB,) =
                    limitOrderQuoter.simulateAToBPriceChange(_amountIn, reserveAIn, reserveBIn, pool, true);
                ...
                bytes memory outputReserveB = scriptRunner.runPythonScript(path, args);
    ...
    function testSimulateAmountOutV3_Fuzz1_ZeroForOneTrue(uint64 _alphaX) public {
        ...
        if (run) {
            ...
            uint160 sqrtPriceLimitX96 = SqrtPriceMath.getNextSqrtPriceFromInput(sqrtPriceX96, liquidity, _alphaX, true);
            uint256 amountOut = iQuoter.quoteExactInputSingle(tokenIn, WETH, 3000, _alphaX, sqrtPriceLimitX96);
            uint256 amountOutMin = limitOrderQuoter.calculateAmountOutMinAToWeth(poolAddress, _alphaX, 0, 3000, tokenIn);
    ...
    function testSimulateAmountOutV3_Fuzz1_ZeroForOneFalse(uint64 _alphaX) public {
        ...
        if (run) {
            ...
            (uint160 sqrtPriceX96,,,,,,) = IUniswapV3Pool(poolAddress).slot0();
            uint128 liquidity = IUniswapV3Pool(poolAddress).liquidity();
            uint160 sqrtPriceLimitX96 = SqrtPriceMath.getNextSqrtPriceFromInput(sqrtPriceX96, liquidity, _alphaX, false);
            uint256 amountOut = iQuoter.quoteExactInputSingle(WETH, tokenOut, 3000, _alphaX, sqrtPriceLimitX96);
            uint256 amountOutMin = limitOrderQuoter.calculateAmountOutMinAToWeth(poolAddress, _alphaX, 0, 3000, WETH);
    ...
    function testSimulateWethToBPriceChangeV2() public {
        ...
        (uint256 newSpotPriceB,,) =
            limitOrderQuoter.simulateWethToBPriceChange(5000000000000000000, tokenToTokenExecutionPrice);
    ...
    function testSimulateAToWethPriceChangeV2() public {
        ...
        (uint256 newSpotPriceA,,, uint128 amountOut) =
            limitOrderQuoter.simulateAToWethPriceChange(50000000000000000000000, tokenToTokenExecutionPrice);
...
contract ExecutionWrapper is LimitOrderQuoter {
    ...
    function simulateAToBPriceChange(
        ...
        address pool,
        bool isTokenToWeth
    ) public returns (uint256, uint128, uint128, uint128) {
        return _simulateAToBPriceChange(alphaX, reserveA, reserveB, pool, isTokenToWeth);
    }

    function simulateAToWethPriceChange(
        uint128 alphaX,
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory executionPrice
    ) public returns (uint256 newSpotPriceA, uint128 newReserveAToken, uint128 newReserveAWeth, uint128 amountOut) {
    ...
    function simulateWethToBPriceChange(
        uint128 alphaX,
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory executionPrice
    ) public returns (uint256 newSpotPriceB, uint128 newReserveBWeth, uint128 newReserveBToken) {
