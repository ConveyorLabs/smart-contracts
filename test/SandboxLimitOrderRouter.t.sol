...
contract SandboxLimitOrderRouterTest is DSTest {
    ...
    address _sushiSwapRouterAddress = 0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F;
    ...
    bytes32 _uniswapV2HexDem = hex"96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f";
    ...
    function setUp() public {
        ...
        sandboxRouter = ISandboxLimitOrderRouter(limitOrderExecutor.SANDBOX_LIMIT_ORDER_ROUTER());

        orderBook = ISandboxLimitOrderBook(limitOrderExecutor.SANDBOX_LIMIT_ORDER_BOOK());
    }
    ...
    function testExecuteMulticallOrderSingleV2() public {
        ...
        SandboxLimitOrderRouter.Call[] memory calls = new SandboxLimitOrderRouter.Call[](3);
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orders = new SandboxLimitOrderBook.SandboxLimitOrder[](1);

        {
            ...
            calls[1] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[0], order.amountInRemaining, order.amountOutRemaining),
                address(this),
                WETH
            );
            ...
            multiCall = newMockMulticall(orderIdBundles, fillAmounts, transferAddress, calls);
        }

        {
            ///@notice Get the Cached balances pre execution

            (uint256 txOriginBalanceBefore, uint256 gasCompensationUpperBound) =
                initializePreSandboxExecutionTxOriginGasCompensationState(orderIds, tx.origin);

            uint256 wethBalanceBefore = IERC20(WETH).balanceOf(address(limitOrderExecutor));
            ...
            validatePostSandboxExecutionGasCompensation(txOriginBalanceBefore, gasCompensationUpperBound);
            for (uint256 i = 0; i < orders.length; ++i) {
                if (orders[i].amountInRemaining == multiCall.fillAmounts[i]) {
                    SandboxLimitOrderBook.OrderType orderType =
                        orderBook.addressToOrderIds(address(this), orders[i].orderId);
                    assert(orderType == SandboxLimitOrderBook.OrderType.FilledSandboxLimitOrder);
                }
            }
    ...
    function testExecuteMulticallOrderSingleV3() public {
        ...
        SandboxLimitOrderBook.SandboxLimitOrder memory order =
            newSandboxLimitOrder(false, 100000000000000000000, 10000000000000000, DAI, WETH);
        SandboxLimitOrderRouter.SandboxMulticall memory multiCall;
        ///@notice Initialize Arrays for Multicall struct.
        bytes32[] memory orderIds = new bytes32[](1);
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orders = new SandboxLimitOrderBook.SandboxLimitOrder[](1);
        {
            ...
            calls[0] = newUniV3Call(
                daiWethV3, address(sandboxRouter), address(sandboxRouter), true, 100000000000000000000, DAI
            );
            calls[1] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[0], order.amountInRemaining, order.amountOutRemaining),
                address(this),
    ...
    function testExecuteMulticallOrderBatch() public {
...

        {
            ...
            for (uint256 i = 0; i < orders.length; ++i) {
                if (orders[i].amountInRemaining == multiCall.fillAmounts[i]) {
                    SandboxLimitOrderBook.OrderType orderType =
                        orderBook.addressToOrderIds(orders[i].owner, orders[i].orderId);

                    assert(orderType == SandboxLimitOrderBook.OrderType.FilledSandboxLimitOrder);
                } else {
                    SandboxLimitOrderBook.SandboxLimitOrder memory order =
                        orderBook.getSandboxLimitOrderById(orders[i].orderId);

                    assertEq(order.amountInRemaining, orders[i].amountInRemaining - multiCall.fillAmounts[i]);

                    assertEq(
                        order.amountOutRemaining,
                        orders[i].amountOutRemaining
                            - ConveyorMath.mul64U(
                                ConveyorMath.divUU(orders[i].amountOutRemaining, orders[i].amountInRemaining),
    ...
    function testExecuteMulticallOrdersSameOwnerBundleInputToken() public {
        ...
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orders = createMockOrdersSameInputToken();
        ...
        bytes32[] memory orderIds = placeMultipleMockOrder(orders);
        {
            ...
            uint256 cumulativeFee = orders[0].feeRemaining + orders[1].feeRemaining;
            ...
            ///@notice Create a single v2 swap call for the multicall.
            calls[0] = newUniV3Call(
                daiWethV3, address(sandboxRouter), address(sandboxRouter), true, 200000000000000000000, DAI
            );

            calls[1] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[0], orders[0].amountInRemaining, orders[0].amountOutRemaining),
                address(this),
                WETH
            );

            calls[2] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[1], orders[1].amountInRemaining, orders[1].amountOutRemaining),
                address(this),
                WETH
            );
            ...
            bytes32[][] memory orderIdBundles = initialize2DimensionalOrderIdBundlesArray();
...

        {
            ...
            for (uint256 i = 0; i < orders.length; ++i) {
                SandboxLimitOrderBook.OrderType orderType = orderBook.addressToOrderIds(address(this), orderIds[i]);
                assert(orderType == SandboxLimitOrderBook.OrderType.FilledSandboxLimitOrder);
            }
        ...
        returns (SandboxLimitOrderBook.SandboxLimitOrder[] memory orders)
    {
        ///@notice Dai/Weth sell limit order
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order0 =
            newSandboxLimitOrder(false, 100000000000000000000, 10000000000000000, DAI, WETH);
        ///@notice Dai/Weth sell limit order
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order1 =
            newSandboxLimitOrder(false, 100000000000000000000, 10000000000000000, DAI, WETH);
    ...
    function initialize2DimensionalOrderIdBundlesArray() internal pure returns (bytes32[][] memory) {
        bytes32[][] memory orderIdBundles = new bytes32[][](1);
    ...
    function testFailExecuteMulticallOrder_FillAmountSpecifiedGreaterThanAmountRemaining() public {
        cheatCodes.deal(address(swapHelper), type(uint256).max);
        ...
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order =
            newSandboxLimitOrder(false, 10000000000000000000, 1, DAI, WETH);
        ...
        SandboxLimitOrderRouter.Call[] memory calls = new SandboxLimitOrderRouter.Call[](2);
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orders = new SandboxLimitOrderBook.SandboxLimitOrder[](1);
        {
            address[] memory transferAddress = new address[](1);
            uint128[] memory fillAmounts = new uint128[](1);
    ...
    function testFailExecuteMulticallOrder_SandboxAmountOutRequiredNotSatisfied() public {
        cheatCodes.deal(address(swapHelper), type(uint256).max);
        ...
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order =
            newSandboxLimitOrder(false, 10000000000000000000, 100000000000000000, DAI, WETH);
    ...
    function testFailExecuteMulticallOrder_InvalidTransferAddressArray() public {
        cheatCodes.deal(address(swapHelper), type(uint256).max);
    ...
    function testInitializeSandboxExecutionState(uint128 wethQuantity, uint128 fillAmountWeth) public {
        bool run;
        uint256 minWethQuantity = 10e15;
        assembly {
            run := and(lt(minWethQuantity, wethQuantity), lt(wethQuantity, 10000000000000000000000))
        }
        if (run) {
            ...
            (bool depositSuccess,) = address(WETH).call{value: wethQuantity}(abi.encodeWithSignature("deposit()"));
            require(depositSuccess, "Fudge");
            IERC20(WETH).approve(address(limitOrderExecutor), wethQuantity);

            ///@notice Dai/Weth sell limit order
            ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
            SandboxLimitOrderBook.SandboxLimitOrder memory orderWeth =
                newSandboxLimitOrder(false, wethQuantity, 1, WETH, DAI);
                ...
                if (fillAmountWeth > wethQuantity) {
                    cheatCodes.expectRevert(
                        abi.encodeWithSelector(
                            Errors.FillAmountSpecifiedGreaterThanAmountRemaining.selector,
                            fillAmountWeth,
                            wethQuantity,
                            orderIds[0]
                        )
                    );
                    (bool reverted,) = address(sandboxLimitOrderBookWrapper).call(
                        abi.encodeWithSignature(
                            "initializePreSandboxExecutionState(bytes32[][],uint128[])", orderIdBundles, fillAmounts
                        )
                    );
                    assertTrue(reverted);
                } else {
                    SandboxLimitOrderBook.PreSandboxExecutionState memory preSandboxExecutionState =
                        sandboxLimitOrderBookWrapper.initializePreSandboxExecutionState(orderIdBundles, fillAmounts);
    ...
    function testValidateSandboxExecutionAndFillOrders(
        ...
        {
            if (
                wethQuantity < 1000000000000000 || daiQuantity < 1000000000000000
                    || wethQuantity > 10000000000000000000000 || daiQuantity > 10000000000000000000000
            ) {
                run = false;
            }
        }
        if (run) {
            if (fillAmountWeth <= wethQuantity && initialBalanceIn < wethQuantity) {
                ...
                ///@notice Swap 1000 Ether into Dai to fund the test contract on the input token
                try swapHelper.swapEthForTokenWithUniV2(daiQuantity, DAI) returns (uint256 amountOut) {
                    {}
                    SandboxLimitOrderBook.SandboxLimitOrder[] memory orders =
                    new SandboxLimitOrderBook.SandboxLimitOrder[](
                            1
                        );
                    ...
                    bytes32[][] memory orderIdBundles = new bytes32[][](1);
                    {
                        ///@notice Dai/Weth sell limit order
                        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
                        orders[0] = newSandboxLimitOrder(false, wethQuantity, amountOutRemaining, WETH, DAI);
    ...
    function validateSandboxExecutionAndFillOrders(
        ...
        SandboxLimitOrderBook.PreSandboxExecutionState memory preSandboxExecutionState = SandboxLimitOrderBook
        ...
        if (initialBalancesIn[0] - wethQuantity > fillAmounts[0] || amountOut < orders[0].amountOutRemaining) {
            cheatCodes.expectRevert(
                abi.encodeWithSelector(
                    ...
                    initialBalancesIn[0] - wethQuantity > fillAmounts[0]
                        ? fillAmounts[0]
                        : ConveyorMath.mul64U(
                            ConveyorMath.divUU(orders[0].amountOutRemaining, orders[0].amountInRemaining), fillAmounts[0]
                        )
                )
            );
            ...
            sandboxLimitOrderBookWrapper.validateSandboxExecutionAndFillOrders(
                orderIdBundles, fillAmounts, preSandboxExecutionState
            );
            {
                SandboxLimitOrderBook.SandboxLimitOrder memory postExecutionOrder =
                    sandboxLimitOrderBookWrapper._getSandboxLimitOrderById(orders[0].orderId);
                if (fillAmounts[0] == orders[0].amountInRemaining) {
                    ...
                    assertEq(postExecutionOrder.amountInRemaining, orders[0].amountInRemaining - fillAmounts[0]);
                    ...
                    assertEq(
                        ...
                        ConveyorMath.mul64U(
                            ConveyorMath.divUU(orders[0].amountOutRemaining, orders[0].amountInRemaining),
                            fillAmounts[0]
                        )
                    );
                    ...
                    assertEq(
                        ...
                        ConveyorMath.mul64U(
                            ConveyorMath.divUU(fillAmounts[0], orders[0].amountInRemaining), orders[0].feeRemaining
                        )
                    );
                }
            }
    ...
    function _requireConveyorFeeIsPaid(
        ...
        ///@notice Wrap the weth to send from the sandboxRouter to the executor in a call.
        (bool depositSuccess,) = address(WETH).call{value: compensationAmount}(abi.encodeWithSignature("deposit()"));
        require(depositSuccess, "Fudge");

        if (contractBalancePreExecution + expectedAccumulatedFees > compensationAmount) {
            cheatCodes.expectRevert(
                abi.encodeWithSelector(
                    Errors.ConveyorFeesNotPaid.selector,
                    expectedAccumulatedFees,
                    IERC20(WETH).balanceOf(address(limitOrderExecutor)) - contractBalancePreExecution,
                    expectedAccumulatedFees
                        - ((compensationAmount + contractBalancePreExecution) - contractBalancePreExecution)
                )
            );
            ...
            contractBalancePreExecution + expectedAccumulatedFees > compensationAmount
                ? assertTrue(reverted)
                : assertTrue(!reverted);
        }
    }
    ...
    function initializePreSandboxExecutionTxOriginGasCompensationState(bytes32[] memory orderIds, address txOrigin)
        internal
        view
        returns (uint256 txOriginBalanceBefore, uint256 gasCompensationUpperBound)
    {
        txOriginBalanceBefore = address(txOrigin).balance;
        for (uint256 i = 0; i < orderIds.length; ++i) {
            SandboxLimitOrderBook.SandboxLimitOrder memory order = orderBook.getSandboxLimitOrderById(orderIds[i]);
    ...
    function validatePostSandboxExecutionGasCompensation(
        ...
        uint256 totalGasCompensated = address(tx.origin).balance - txOriginBalanceBefore;
    ...
    function validatePostExecutionProtocolFees(
        ...
        uint256 feesCompensated = IERC20(WETH).balanceOf(address(limitOrderExecutor)) - wethBalanceBefore;
    ...
    function _calculateExactAmountRequired(uint256 amountFilled, uint256 amountInRemaining, uint256 amountOutRemaining)
        internal
        pure
        returns (uint256 amountOutRequired)
    {
        amountOutRequired = ConveyorMath.mul64U(ConveyorMath.divUU(amountOutRemaining, amountInRemaining), amountFilled);
    }

    function initialize10DimensionalOrderIdBundles() internal pure returns (bytes32[][] memory) {
    ...
    function dealSandboxRouterExecutionFee() internal {
        ...
        ///@notice Wrap the weth to send from the sandboxRouter to the executor in a call.
        (bool depositSuccess,) = address(WETH).call{value: 500000 ether}(abi.encodeWithSignature("deposit()"));
        require(depositSuccess, "Fudge");
    }
        ...
        )
    {
        ...
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orders = new SandboxLimitOrderBook.SandboxLimitOrder[](10);
        {
            for (uint256 i = 0; i < orderIds.length; ++i) {
                SandboxLimitOrderBook.SandboxLimitOrder memory order = orderBook.getSandboxLimitOrderById(orderIds[i]);
        ...
        SandboxLimitOrderRouter.Call[] memory calls = new SandboxLimitOrderRouter.Call[](13);
        bytes32[][] memory orderIdBundles = initialize10DimensionalOrderIdBundles();
        {
            orderIdBundles[0][0] = orderIds[0];
            ...
            address daiWethV3 = 0xC2e9F25Be6257c210d7Adf0D4Cd6E3E881ba25f8;
            calls[0] = newUniV3Call(
                daiWethV3, address(sandboxRouter), address(sandboxRouter), true, 110000000000000000000, DAI
            );
            ...
            calls[4] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[2], orders[2].amountInRemaining, orders[2].amountOutRemaining),
                address(mockOwner3),
                WETH
            );
            calls[5] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[3], orders[3].amountInRemaining, orders[3].amountOutRemaining),
                address(mockOwner4),
                WETH
            );
            calls[6] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[4], orders[4].amountInRemaining, orders[4].amountOutRemaining),
                address(mockOwner5),
                WETH
            );
            calls[7] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[5], orders[5].amountInRemaining, orders[5].amountOutRemaining),
                address(mockOwner6),
                WETH
            );
            calls[8] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[6], orders[6].amountInRemaining, orders[6].amountOutRemaining),
                address(mockOwner7),
                WETH
            );
            calls[9] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[7], orders[7].amountInRemaining, orders[7].amountOutRemaining),
                address(mockOwner8),
                WETH
            );

            calls[10] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[8], orders[8].amountInRemaining, orders[8].amountOutRemaining),
                address(mockOwner9),
                DAI
            );
            calls[11] = amountOutRequiredCompensationCall(
                _calculateExactAmountRequired(fillAmounts[9], orders[9].amountInRemaining, orders[9].amountOutRemaining),
                address(mockOwner10),
                DAI
            );
            ...
            SandboxLimitOrderRouter.SandboxMulticall memory multiCall =
                newMockMulticall(orderIdBundles, fillAmounts, transferAddresses, calls);
            return (multiCall, orders, orderIds);
        }
    }

    ///@notice Helper function to create call to compensate the fees during execution
    function feeCompensationCall(uint256 cumulativeFee) public view returns (SandboxLimitOrderRouter.Call memory) {
        bytes memory callData =
            abi.encodeWithSignature("transfer(address,uint256)", address(limitOrderExecutor), cumulativeFee);
    ...
    function amountOutRequiredCompensationCall(uint256 amountOutRequired, address receiver, address tokenOut)
        public
        pure
        returns (SandboxLimitOrderRouter.Call memory)
    {
        bytes memory callData =
            abi.encodeWithSignature("transfer(address,uint256)", address(receiver), amountOutRequired);
        return SandboxLimitOrderRouter.Call({target: tokenOut, callData: callData});
    }

    ///@notice Helper function to create a single mock call for a v2 swap.
    function newUniV2Call(address _lp, uint256 amount0Out, uint256 amount1Out, address _receiver)
        public
        pure
        returns (SandboxLimitOrderRouter.Call memory)
    {
        bytes memory callData = abi.encodeWithSignature(
            "swap(uint256,uint256,address,bytes)", amount0Out, amount1Out, _receiver, new bytes(0)
        );
    ...
    function newMockMulticall(
        ...
        return SandboxLimitOrderRouter.SandboxMulticall({
    ...
    function placeMockOrderWrapper(SandboxLimitOrderBook.SandboxLimitOrder memory order)
        internal
        returns (bytes32 orderId)
    {
        //create a new array of orders
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orderGroup = new SandboxLimitOrderBook.SandboxLimitOrder[](
                1
            );
        ...
        //place order
        bytes32[] memory orderIds =
            sandboxLimitOrderBookWrapper.placeSandboxLimitOrder{value: type(uint64).max}(orderGroup);
    ...
    function newSandboxLimitOrder(
        ...
        address tokenIn,
        address tokenOut
    ) internal view returns (SandboxLimitOrderBook.SandboxLimitOrder memory order) {
    ...
    function placeMockSandboxLimitOrder(SandboxLimitOrderBook.SandboxLimitOrder memory order)
        internal
        returns (bytes32 orderId)
    {
        //create a new array of orders
        ...
        order.executionCreditRemaining = type(uint64).max;
        //place order
        bytes32[] memory orderIds = orderBook.placeSandboxLimitOrder{value: type(uint64).max}(orderGroup);

        orderId = orderIds[0];
    }

    function placeMultipleMockOrder(SandboxLimitOrderBook.SandboxLimitOrder[] memory orderGroup)
        internal
        returns (bytes32[] memory orderIds)
    {
        ...
        //place order
        orderIds = orderBook.placeSandboxLimitOrder{value: type(uint32).max * orderGroup.length}(orderGroup);
    }

    function placeNewMockMultiOrderMultiCall() internal returns (bytes32[] memory) {
        ...
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order1 =
            newSandboxLimitOrder(false, 120000000000000000000000, 100000000000000000000, DAI, WETH);
        ...
        ///@notice Dai/Weth sell limit order
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order2 =
            newSandboxLimitOrder(false, 120000000000000000000000, 100000000000000000000, DAI, WETH);
        ...
        ///@notice Dai/Weth sell limit order
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order3 =
            newSandboxLimitOrder(false, 100000000000000000000, 10000000000000000, DAI, WETH);
        ...
        ///@notice Dai/Weth sell limit order
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order4 =
            newSandboxLimitOrder(false, 100000000000000000000, 10000000000000000, DAI, WETH);
        ...
        ///@notice Dai/Weth sell limit order
        ///@dev amountInRemaining 1000 DAI amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order5 =
            newSandboxLimitOrder(false, 100000000000000000000, 10000000000000000, DAI, WETH);
        ...
        SandboxLimitOrderBook.SandboxLimitOrder memory order6 =
            newSandboxLimitOrder(false, 10000000000, 10000000000000000, USDC, WETH);
        ...
        ///@notice USDC/Weth sell limit order
        ///@dev amountInRemaining 10000 USDC amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order7 =
            newSandboxLimitOrder(false, 10000000000, 10000000000000000, USDC, WETH);
        ...
        ///@notice USDC/Weth sell limit order
        ///@dev amountInRemaining 10000 USDC amountOutRemaining 1 Wei
        SandboxLimitOrderBook.SandboxLimitOrder memory order8 =
            newSandboxLimitOrder(false, 10000000000, 10000000000000000, USDC, WETH);
        ...
        SandboxLimitOrderBook.SandboxLimitOrder memory order9 =
            newSandboxLimitOrder(true, 100000000000000000000, 120000000000000000000000, WETH, DAI);
...

        {
            cheatCodes.prank(mockOwner10);
            ///@notice Wrap the weth to send to the executor in a call.
            (bool depositSuccess,) = address(WETH).call{value: 1000000 ether}(abi.encodeWithSignature("deposit()"));
            require(depositSuccess, "fudge");
        }
        ///@notice Weth/Dai sell limit order
        ///@dev amountInRemaining 1000 WETH amountOutRemaining 120000.0 DAI
        SandboxLimitOrderBook.SandboxLimitOrder memory order10 =
            newSandboxLimitOrder(true, 100000000000000000000, 120000000000000000000000, WETH, DAI);
        {
    ...
    function placeMockOrderCustomOwner(SandboxLimitOrderBook.SandboxLimitOrder memory order, address owner)
        internal
        returns (bytes32 orderId)
    {
        //create a new array of orders
    ...
    function placeMockOrder(SandboxLimitOrderBook.SandboxLimitOrder memory order) internal returns (bytes32 orderId) {
        //create a new array of orders
        SandboxLimitOrderBook.SandboxLimitOrder[] memory orderGroup = new SandboxLimitOrderBook.SandboxLimitOrder[](
                1
            );
...
contract LimitOrderExecutorWrapper is ConveyorExecutor {
    ...
    ) ConveyorExecutor(_weth, _usdc, _limitOrderQuoter, _dexFactories, _isUniV2, _executionCredit) {}

    function requireConveyorFeeIsPaid(uint256 contractBalancePreExecution, uint256 expectedAccumulatedFees)
        public
        view
    {
...
contract SandboxLimitOrderBookWrapper is SandboxLimitOrderBook {
    constructor(address _limitOrderExecutor, address _weth, address _usdc, uint256 _minExecutionCredit)
        SandboxLimitOrderBook(_limitOrderExecutor, _weth, _usdc, _minExecutionCredit)
    {}

    function _getSandboxLimitOrderById(bytes32 orderId) public view returns (SandboxLimitOrder memory) {
        return getSandboxLimitOrderById(orderId);
    }

    function initializePreSandboxExecutionState(bytes32[][] calldata orderIdBundles, uint128[] calldata fillAmounts)
        public
        view
    ...
    function validateSandboxExecutionAndFillOrders(
        ...
        _validateSandboxExecutionAndFillOrders(orderIdBundles, fillAmounts, preSandboxExecutionState);
    }
    ...
    function executeSandboxLimitOrders(
        ...
        IConveyorExecutor(address(limitOrderExecutor)).executeSandboxLimitOrders(orders, sandboxMulticall);
    }
}
