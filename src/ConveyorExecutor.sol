...
contract ConveyorExecutor is IConveyorExecutor, LimitOrderSwapRouter {
    ...
    modifier onlyOrderBook() {
        if (msg.sender != LIMIT_ORDER_ROUTER && msg.sender != SANDBOX_LIMIT_ORDER_BOOK) {
    ...
    constructor(
        ...
        SANDBOX_LIMIT_ORDER_ROUTER =
            ISandboxLimitOrderBook(SANDBOX_LIMIT_ORDER_BOOK).getSandboxLimitOrderRouterAddress();
    ...
    function executeTokenToWethOrders(LimitOrderBook.LimitOrder[] calldata orders)
        external
        ...
        returns (uint256, uint256)
    {
        ...
        (SpotReserve[] memory spotReserveAToWeth, address[] memory lpAddressesAToWeth) =
            getAllPrices(orders[0].tokenIn, WETH, orders[0].feeIn);

        ///@notice Initialize all execution prices for the token pair.
        TokenToWethExecutionPrice[] memory executionPrices = ILimitOrderQuoter(LIMIT_ORDER_QUOTER)
            .initializeTokenToWethExecutionPrices(spotReserveAToWeth, lpAddressesAToWeth);
        ...
        for (uint256 i = 0; i < orders.length;) {
            ...
            uint256 bestPriceIndex =
                ILimitOrderQuoter(LIMIT_ORDER_QUOTER).findBestTokenToWethExecutionPrice(executionPrices, orders[i].buy);

            {
                ///@notice Pass the order, maxBeaconReward, and TokenToWethExecutionPrice into _executeTokenToWethSingle for execution.
                (uint256 conveyorReward, uint256 beaconReward) =
                    _executeTokenToWethOrder(orders[i], executionPrices[bestPriceIndex]);
            ...
            executionPrices[bestPriceIndex] = ILimitOrderQuoter(LIMIT_ORDER_QUOTER).simulateTokenToWethPriceChange(
                uint128(orders[i].quantity), executionPrices[bestPriceIndex]
            );
    ...
    function _executeTokenToWethOrder(
        ...
        (uint128 amountOutWeth, uint128 conveyorReward, uint128 beaconReward) =
            _executeSwapTokenToWethOrder(executionPrice.lpAddressAToWeth, order);
    ...
    function _executeSwapTokenToWethOrder(address lpAddressAToWeth, LimitOrderBook.LimitOrder calldata order)
        internal
        returns (uint128 amountOutWeth, uint128 conveyorReward, uint128 beaconReward)
    {
        ...
        uint256 amountOutMinAToWeth = ILimitOrderQuoter(LIMIT_ORDER_QUOTER).calculateAmountOutMinAToWeth(
            lpAddressAToWeth, orderQuantity, order.taxIn, feeIn, tokenIn
        );

        ///@notice Swap from tokenA to Weth.
        amountOutWeth = uint128(
            _swap(
                tokenIn, WETH, lpAddressAToWeth, feeIn, orderQuantity, amountOutMinAToWeth, address(this), order.owner
            )
        );
        ...
        (conveyorReward, beaconReward) = ConveyorFeeMath.calculateReward(protocolFee, amountOutWeth);
    ...
    function executeTokenToTokenOrders(LimitOrderBook.LimitOrder[] calldata orders)
        external
        onlyLimitOrderRouter
        returns (uint256, uint256)
    {
...

        {
            ...
            (SpotReserve[] memory spotReserveWethToB, address[] memory lpAddressWethToB) =
                getAllPrices(WETH, orders[0].tokenOut, feeOut);

            executionPrices = ILimitOrderQuoter(LIMIT_ORDER_QUOTER).initializeTokenToTokenExecutionPrices(
                tokenIn, spotReserveAToWeth, lpAddressesAToWeth, spotReserveWethToB, lpAddressWethToB
            );
        }
        ...
        for (uint256 i = 0; i < orders.length;) {
            ///@notice Create a variable to track the best execution price in the array of execution prices.
            uint256 bestPriceIndex =
                ILimitOrderQuoter(LIMIT_ORDER_QUOTER).findBestTokenToTokenExecutionPrice(executionPrices, orders[i].buy);

            {
                ///@notice Pass the order, maxBeaconReward, and TokenToWethExecutionPrice into _executeTokenToWethSingle for execution.
                (uint256 conveyorReward, uint256 beaconReward) =
                    _executeTokenToTokenOrder(orders[i], executionPrices[bestPriceIndex]);
                totalBeaconReward += beaconReward;
                totalConveyorReward += conveyorReward;
            }

            ///@notice Update the best execution price.
            executionPrices[bestPriceIndex] = ILimitOrderQuoter(LIMIT_ORDER_QUOTER).simulateTokenToTokenPriceChange(
                uint128(orders[i].quantity), executionPrices[bestPriceIndex]
            );
    ...
    function _executeTokenToTokenOrder(
        ...
        ///@notice Scope to prevent stack too deep.
        {
            ...
            if (order.tokenIn != WETH) {
                ...
                (amountInWethToB, conveyorReward, beaconReward) =
                    _executeSwapTokenToWethOrder(executionPrice.lpAddressAToWeth, order);
                ...
                uint128 protocolFee = calculateFee(uint128(amountIn), USDC, WETH);

                ///@notice Calculate the conveyorReward and the off-chain logic executor reward.
                (conveyorReward, beaconReward) = ConveyorFeeMath.calculateReward(protocolFee, uint128(amountIn));
    ...
    function _transferTokensToContract(LimitOrderBook.LimitOrder calldata order) internal {
        IERC20(order.tokenIn).safeTransferFrom(order.owner, address(this), order.quantity);
    }
    ...
    function executeSandboxLimitOrders(
        ...
        if (sandboxMulticall.transferAddresses.length > 0) {
            ...
            for (uint256 i = 0; i < orders.length;) {
                ...
                IERC20(orders[i].tokenIn).safeTransferFrom(
                    orders[i].owner, sandboxMulticall.transferAddresses[i], fillAmount
                );

                uint256 feeRequired = ConveyorMath.mul64U(
                    ConveyorMath.divUU(fillAmount, orders[i].amountInRemaining), orders[i].feeRemaining
                );

                if (feeRequired == 0) {
                    revert InsufficientFillAmountSpecified(fillAmount, orders[i].amountInRemaining);
                }
            ...
            for (uint256 i = 0; i < orders.length;) {
                uint128 fillAmount = sandboxMulticall.fillAmounts[i];
                IERC20(orders[i].tokenIn).safeTransferFrom(orders[i].owner, SANDBOX_LIMIT_ORDER_ROUTER, fillAmount);
                uint256 feeRequired = ConveyorMath.mul64U(
                    ConveyorMath.divUU(fillAmount, orders[i].amountInRemaining), orders[i].feeRemaining
                );
        ...
        ISandboxLimitOrderRouter(SANDBOX_LIMIT_ORDER_ROUTER).sandboxRouterCallback(sandboxMulticall);
    ...
    function _requireConveyorFeeIsPaid(uint256 contractBalancePreExecution, uint256 expectedAccumulatedFees)
        internal
        view
    {
        ...
        assembly {
            feeIsPaid :=
                iszero(lt(contractBalancePostExecution, add(contractBalancePreExecution, expectedAccumulatedFees)))
        }
