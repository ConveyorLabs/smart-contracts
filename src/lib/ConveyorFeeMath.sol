...
library ConveyorFeeMath {
        ...
        returns (uint128 conveyorReward, uint128 beaconReward)
    {
        ...
        uint256 totalWethReward = ConveyorMath.mul64U(percentFee, uint256(wethValue));
        ...
        if (percentFee <= ZERO_POINT_ZERO_ZERO_FIVE) {
            int256 innerPartial = int256(uint256(ZERO_POINT_ZERO_ZERO_FIVE)) - int128(percentFee);
        ...
        conveyorReward = uint128(ConveyorMath.mul64U(conveyorPercent, totalWethReward));
