...
contract ConveyorTickMathTest is DSTest {
    ...
    function testSimulateAmountOutOnSqrtPriceX96__ZeroForOneFalse(uint128 _alphaX) public {
        bool run = true;
        if (_alphaX == 0) {
            run = false;
        }
        ...
        if (run) {
            ...
            uint256 amountOutExpected =
                iQuoter.quoteExactInputSingle(WETH, DAI, 3000, _alphaX, TickMath.MAX_SQRT_RATIO - 1);
            (uint128 amountOutToValidate,) = conveyorTickMath._simulateAmountOutOnSqrtPriceX96(
                token0, WETH, daiWethPoolV3, _alphaX, tickSpacing, liquidity, 3000
            );
    ...
    function testSimulateAmountOutOnSqrtPriceX96__ZeroForOneTrue(uint128 _alphaX) public {
        if (_alphaX > 0) {
            int24 tickSpacing = IUniswapV3Pool(daiWethPoolV3).tickSpacing();
            address token0 = IUniswapV3Pool(daiWethPoolV3).token0();
            uint128 liquidity = IUniswapV3Pool(daiWethPoolV3).liquidity();
            ...
            //Get the expected amountOut in Dai from the v3 quoter.
            uint256 amountOutExpected =
                iQuoter.quoteExactInputSingle(DAI, WETH, 3000, _alphaX, TickMath.MIN_SQRT_RATIO + 1);

            //Get the quoted amount out from _simulateAmountOutOnSqrtPriceX96.
            (uint128 amountOutToValidate,) = conveyorTickMath._simulateAmountOutOnSqrtPriceX96(
                token0, DAI, daiWethPoolV3, _alphaX, tickSpacing, liquidity, 3000
            );
    ...
    function testSimulateAmountOutOnSqrtPriceX96CrossTick(uint128 _alphaX) public {
        bool run = true;
        if (_alphaX == 0) {
            run = false;
        }
        if (run) {
            ...
            (uint128 amountOutToValidate,) = conveyorTickMath._simulateAmountOutOnSqrtPriceX96(
                token0, WETH, usdcWethPoolV3, _alphaX, tickSpacing, liquidity, 500
            );

            //Get the expected amountOut in Dai from the v3 quoter.
            uint256 amountOutExpected =
                iQuoter.quoteExactInputSingle(WETH, USDC, 500, _alphaX, TickMath.MAX_SQRT_RATIO - 1);
...
contract ConveyorTickMathWrapper is ConveyorTickMath {
    function _fromSqrtX96(uint160 sqrtPriceX96, bool token0IsReserve0, address token0, address token1)
        public
        view
    ...
    function _simulateAmountOutOnSqrtPriceX96(
        ...
        return simulateAmountOutOnSqrtPriceX96(token0, tokenIn, lpAddressAToWeth, amountIn, tickSpacing, liquidity, fee);
    }
}
