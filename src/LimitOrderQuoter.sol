...
contract LimitOrderQuoter is ILimitOrderQuoter, ConveyorTickMath {
    ...
    function findBestTokenToWethExecutionPrice(
        LimitOrderSwapRouter.TokenToWethExecutionPrice[] calldata executionPrices,
        bool buyOrder
    ...
    function findBestTokenToTokenExecutionPrice(
        LimitOrderSwapRouter.TokenToTokenExecutionPrice[] calldata executionPrices,
        bool buyOrder
    ) external pure returns (uint256 bestPriceIndex) {
    ...
    function initializeTokenToWethExecutionPrices(
    ...
    ) external pure returns (LimitOrderSwapRouter.TokenToWethExecutionPrice[] memory) {
        ...
        new LimitOrderSwapRouter.TokenToWethExecutionPrice[](
        ...
        ///@notice Scoping to avoid stack too deep.
        {
            ...
            for (uint256 i = 0; i < spotReserveAToWeth.length;) {
                executionPrices[i] = LimitOrderSwapRouter.TokenToWethExecutionPrice(
    ...
    function initializeTokenToTokenExecutionPrices(
        ...
        new LimitOrderSwapRouter.TokenToTokenExecutionPrice[](
        ...
        if (tokenIn == WETH) {
            ...
            for (uint256 i = 0; i < spotReserveWethToB.length;) {
                ...
                executionPrices[i] = LimitOrderSwapRouter.TokenToTokenExecutionPrice(
                    0,
                    0,
                ...
                for (uint256 j = 0; j < spotReserveWethToB.length;) {
                    ...
                    uint256 spotPriceFinal = uint256(
                        _calculateTokenToWethToTokenSpotPrice(
                            spotReserveAToWeth[i].spotPrice, spotReserveWethToB[j].spotPrice
                        )
                    ) << 64;

                    ///@notice Set the executionPrices at index to TokenToTokenExecutionPrice
                    executionPrices[index] = LimitOrderSwapRouter.TokenToTokenExecutionPrice(
                        spotReserveAToWeth[i].res0,
                        spotReserveAToWeth[i].res1,
    ...
    function simulateTokenToTokenPriceChange(
        ...
        if (executionPrice.aToWethReserve0 != 0 && executionPrice.aToWethReserve1 != 0) {
            ...
            uint8 tokenInDecimals = token1 == WETH ? IERC20(token0).decimals() : IERC20(token1).decimals();
            ...
            executionPrice = _simulateTokenToTokenPriceChange(amountIn, executionPrice);
        } else {
            ///@notice Abstracted function call to simulate the weth to token price change on the common decimal amountIn
            executionPrice = _simulateWethToTokenPriceChange(alphaX, executionPrice);
        }
    ...
    function _simulateTokenToTokenPriceChange(
        uint128 alphaX,
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory executionPrice
    ) internal returns (LimitOrderSwapRouter.TokenToTokenExecutionPrice memory) {
        ///@notice Retrive the new simulated spot price, reserve values, and amount out on the TokenIn To Weth pool
        (uint256 newSpotPriceA, uint128 newReserveAToken, uint128 newReserveAWeth, uint128 amountOut) =
        ...
        (uint256 newSpotPriceB, uint128 newReserveBToken, uint128 newReserveBWeth) =
...

        {
            ...
            uint256 newTokenToTokenSpotPrice =
                uint256(ConveyorMath.mul64x64(uint128(newSpotPriceA >> 64), uint128(newSpotPriceB >> 64))) << 64;
    ...
    function _simulateAToWethPriceChange(
        uint128 alphaX,
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory executionPrice
    ) internal returns (uint256 newSpotPriceA, uint128 newReserveAToken, uint128 newReserveAWeth, uint128 amountOut) {
        ...
        (newSpotPriceA, newReserveAToken, newReserveAWeth, amountOut) =
            _simulateAToBPriceChange(alphaX, reserveAToken, reserveAWeth, poolAddressAToWeth, true);
    }
    ...
    function _simulateWethToTokenPriceChange(
        ...
        (uint256 newSpotPriceB, uint128 newReserveBWeth, uint128 newReserveBToken,) =
            _simulateAToBPriceChange(alphaX, reserveBWeth, reserveBToken, poolAddressWethToB, false);
    ...
    function _simulateWethToBPriceChange(
        uint128 alphaX,
        LimitOrderSwapRouter.TokenToTokenExecutionPrice memory executionPrice
    ) internal returns (uint256 newSpotPriceB, uint128 newReserveBWeth, uint128 newReserveBToken) {
        ...
        (newSpotPriceB, newReserveBWeth, newReserveBToken,) =
            _simulateAToBPriceChange(alphaX, reserveBToken, reserveBWeth, poolAddressWethToB, false);
    }
    ...
    function _simulateAToBPriceChange(
        ...
        address pool,
        bool isTokenToWeth
    ) internal returns (uint256, uint128, uint128, uint128) {
        ...
        if (_lpIsNotUniV3(pool)) {
            unchecked {
                ...
                uint256 spotPrice = uint256(ConveyorMath.divUU(numerator, denominator)) << 64;
                ...
                uint128 amountOut = uint128(getAmountOut(alphaX, reserveA, reserveB));
            ...
            (uint128 spotPrice64x64, uint128 amountOut) = calculateNextSqrtPriceX96(isTokenToWeth, pool, alphaX);
    ...
    function simulateTokenToWethPriceChange(
        ...
        (executionPrice.price, executionPrice.aToWethReserve0, executionPrice.aToWethReserve1,) =
        _simulateAToBPriceChange(amountIn, executionPrice.aToWethReserve0, executionPrice.aToWethReserve1, pool, true);
    ...
    function calculateNextSqrtPriceX96(bool isTokenToWeth, address pool, uint256 alphaX)
        internal
        view
        returns (uint128 spotPrice, uint128 amountOut)
    {
        ...
        spotPrice = uint128(fromSqrtX96(price, wethIsToken0, token0, token1) >> 64);
    }
    ...
    function calculateAmountOutMinAToWeth(
        ...
        if (!_lpIsNotUniV3(lpAddressAToWeth)) {
            ...
            (amountOutMinAToWeth,) = ConveyorTickMath.simulateAmountOutOnSqrtPriceX96(
                token0, tokenIn, lpAddressAToWeth, amountIn, tickSpacing, liquidity, feeIn
            );
        } else {
            ...
            if (WETH == IUniswapV2Pair(lpAddressAToWeth).token0()) {
                ...
                uint256 amountIn = amountInOrder - amountInBuffer;
                amountOutMinAToWeth = getAmountOut(amountIn, uint256(reserve1), uint256(reserve0));
            } else {
                uint256 amountInBuffer = (amountInOrder * taxIn) / 10 ** 5;

                uint256 amountIn = amountInOrder - amountInBuffer;
                amountOutMinAToWeth = getAmountOut(amountIn, uint256(reserve0), uint256(reserve1));
            }
        }
    }
    ...
    function _calculateTokenToWethToTokenSpotPrice(uint256 spotPriceAToWeth, uint256 spotPriceWethToB)
        internal
        pure
        returns (uint128 spotPriceFinal)
    {
