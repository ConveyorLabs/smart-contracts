...
contract ConveyorRouterV1 {
    ...
    function setAffiliate(address affiliateAddress) external onlyOwner {
    ...
    function initializeReferrer() external payable {
        uint16 memAffiliateNonce = affiliateNonce;

        if (
            msg.value <
            ConveyorMath.mul64U(ONE_TENTH_PERCENT, memAffiliateNonce)
        ) {
            revert InvalidReferralFee();
        }

        affiliates[memAffiliateNonce] = msg.sender;
