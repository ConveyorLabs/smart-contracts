...
contract TokenToTokenLimitOrderExecutionTest is DSTest {
    ...
    function testExecuteTokenToWethOrderBatch() public {
        ...
        IERC20(DAI).approve(address(limitOrderRouter), MAX_UINT);
        ...
        for (uint256 i = 0; i < tokenToWethOrderBatch.length; ++i) {
            OrderBook.Order memory order = limitOrderRouter
    ...
    function testExecuteWethToTokenSingle() public {
        ...
        IERC20(WETH).approve(address(limitOrderRouter), MAX_UINT);
    ...
    function testExecuteTokenToTokenBatch() public {
        ...
        IERC20(USDC).approve(address(limitOrderRouter), MAX_UINT);
    ...
    function testExecuteWethToTaxedTokenSingle() public {
        ...
        //check that the orders have been placed
        for (uint256 i = 0; i < orderBatch.length; ++i) {
            OrderBook.Order memory order1 = limitOrderRouter
                .getOrderById(orderBatch[i]);
    ...
    function testExecuteTaxedTokenToWethSingle() public {
        ...
        IERC20(TAXED_TOKEN).approve(address(limitOrderRouter), MAX_UINT);
        ...
        orderGroup[0] = order;
        bytes32[] memory orderBatch = limitOrderRouter.placeOrder(
            orderGroup
        );
    ...
    function testFindBestTokenToTokenExecutionPrice() public {
        ...
        uint256 bestPriceIndexBuy = limitOrderRouter
            .findBestTokenToTokenExecutionPrice(executionPrices, true);
        uint256 bestPriceIndexSell = limitOrderRouter
        ...
        public
    {
        ...
        if (!underflow) {
            if (_amountIn != 0) {
                (, uint128 reserveA, uint128 reserveB, ) = limitOrderRouter
    ...
    function testSimulateAToBPriceChangeV2SpotPrice(uint112 _amountIn) public {
        ...
        if (!underflow) {
            if (_amountIn != 0) {
                (uint256 spotPrice, , , ) = limitOrderRouter
                    .simulateAToBPriceChange(
                        _amountIn,
                        reserveAIn,
                        reserveBIn,
